...
export const AdminDashboardScreen = () => {
  ...
  const [stats, setStats] = useState({
    mostSellingBranches: [] as StatItem[],
  ...
  const fetchStats = async () => {
    try {
      const [branches, marketers, customers, products] = await Promise.all([
        dashboardApi.getAdminMostSellingBranches(),
        dashboardApi.getAdminMostOrderingMarketers(),
        dashboardApi.getAdminMostOrderingCustomers(),
        dashboardApi.getAdminMostSoldProducts(),
      ]);

      setStats({
        mostSellingBranches: branches,
                ...
                <View
                  style={[
                    styles.progressBar,
                    {
                      width: `${percentage}%`,
                      backgroundColor:
                        colors.primary + (colors.isDark ? "20" : "15"),
                    },
                  ]}
                />
                ...
                <View style={styles.rowContent}>
                  <View style={styles.labelWrapper}>
                    <Text
                      ...
                      numberOfLines={1}
                    >
                      {index === 0 && <Text style={{ fontSize: 14 }}>🏆 </Text>}
        ...
        {loading ? (
          ...
          <View style={styles.gridContainer}>
            {renderStatCard(
              ...
              stats.mostSellingBranches,
              "governorate",
              "orders_count",
              "طلب",
            )}
            {renderStatCard(
              "المسوقين الأكثر طلباً",
